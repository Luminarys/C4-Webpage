...
<?php
	...
	$query = $db->prepare("SELECT MapCompare.prefix, pfx_compare, MapReference.display_name FROM MapCompare LEFT JOIN MapReference ON MapCompare.pfx_compare = MapReference.prefix WHERE pfx_control = ?");
	if($query->execute(array($_GET["spec"]))){
		echo "<select id='compareSelect'>";
		$result = $query->fetchAll();
		foreach ($result as $spec){
			echo "<option prefix='" . $spec["prefix"] . "' value=" . $spec["pfx_compare"] . ">" . $spec["display_name"] . "</option>";
		}
		echo "</select>";
	}else{
